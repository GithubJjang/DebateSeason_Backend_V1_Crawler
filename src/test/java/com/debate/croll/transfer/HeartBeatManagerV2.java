...
public class HeartBeatManagerV2 {
	...
	private SseHeartbeatSenderV2 sseHeartbeatSenderV2;

	public HeartBeatManagerV2(){
		sseHeartbeatSenderV2 = new SseHeartbeatSenderV2();
	}
	...
	public void increment(){
		...
		if(userCount.incrementAndGet()==1){ // 만약 반환해서 처리를 할 경우, 동시성 문제가 발생할 수 있기 때문에 즉시 처리!
			...
			scheduledTask = executor.scheduleAtFixedRate(
				...
				()-> {
					sseHeartbeatSenderV2.sendHeartbeat();
					System.out.println("log1");

					for(int i=0; i<5; i++){
						Record record = new Record("test",count++, Type.Community);
						record.recordFile();
					}
